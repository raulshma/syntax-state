...
Memory leaks work the same way — your app keeps references to data it no longer needs, and eventually runs out of memory.
...
// Check: Chrome DevTools → Performance Monitor → JS Heap Size
...
// ✅ FIX: Clear references when removing DOM
function clearList() {
  document.getElementById('list').innerHTML = '';
...
class MemoryMonitor {
  ...
  takeSnapshot(label = 'Snapshot') {
    if (typeof performance !== 'undefined' && performance.memory) {
      ...
      console.log(\` \${label}: \${snapshot.usedMB} MB\`);
      return snapshot;
    } else {
