...
The janitor doesn't clean desks that employees are still using. Instead, they look for desks that are **completely abandoned** — no employee can reach them anymore. Those get cleaned up!
...
obj = null;                   // Reference count: 0 → Collect!
```
...
function removeReference(id) {
  ...
  if (refCount[id] === 0) {
    console.log(\`🗑️ '\${id}' collected! (0 references)\`);
    delete refCount[id];
  }
}
...
Reference counting has a fatal flaw — **circular references**:
...
**Phase 1: Mark** — Start from "roots" (global objects, current function scope) and mark everything reachable

**Phase 2: Sweep** — Delete everything that wasn't marked
...
// Both are reachable from global → MARKED → KEPT

user = null;
// { name: "Alice" } is now unreachable → NOT MARKED → SWEPT
```
...
AFTER MARK-AND-SWEEP:
  ...
  Global (ROOT)                     
      │                             
      ├──▶ user ──▶ { name: "Alice" }  ✓ Kept
      │                             
      └──▶ settings ──▶ { theme: "dark" }  ✓ Kept
                                    
  🗑️ Circular objects collected!    
```
...
function demonstrateMarkAndSweep() {
  ...
  console.log("Created circular chain: A → B → C → A");
...
- **Call stack** — all functions currently being executed
...
| **Reachability** | Core concept — can you reach it from roots? |
