...
```mermaid
flowchart LR
    subgraph Vitest["fa:fa-flask Vitest Advantages"]
        A["fa:fa-rocket Instant startup<br />(uses Vite)"]
        B["fa:fa-bolt Super fast HMR"]
        C["fa:fa-box ESM out of the box"]
        D["fa:fa-bullseye Jest-compatible API"]
        E["fa:fa-wrench Zero config"]
    end
```
...
- ✅ "Is 2 + 2 equal to 4?" → PASS
- ❌ "Is 2 + 2 equal to 5?" → FAIL
...
**Great job!** 🎉 You've learned the fundamentals of unit testing with Vitest. In the intermediate level, we'll explore mocking, async testing, and more advanced patterns.
