...
Sometimes you don't want to replace a function entirely—you just want to watch it. That's where `vi.spyOn()` comes in:
...
**Excellent progress!**  You now have the tools to test real-world code with dependencies, async operations, and timers. In the advanced level, we'll explore test coverage, property-based testing, and advanced patterns.
