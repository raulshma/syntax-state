...
Think of component testing like test-driving a car. You don't just test the engine in isolation—you sit in the driver's seat and make sure everything works together the way a real driver would use it.

<InfoBox type="tip" title="User-Centric Testing">
Testing Library's philosophy is simple: test your components the way users interact with them. Don't test implementation details—test behavior!
</InfoBox>
...
If a user can't see or interact with something, your test shouldn't either. Query elements the way users find them—by visible text, labels, and roles.
</KeyConcept>
...
Test IDs don't reflect how users interact with your app. If you need a test ID, it might indicate an accessibility issue—users and assistive tech can't find the element either!
</InfoBox>
...
Query elements the way users find them—by role, label, and text. Avoid implementation details like class names and IDs.
</KeyConcept>
...
**Great start!**  You've learned the fundamentals of component testing. In the intermediate level, we'll explore testing forms, async operations, and component state.
