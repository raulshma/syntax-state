...
|:---------|:--------|:--------|
| `parseInt(str, radix)` | String → Integer | `parseInt("42")` → `42` |
| `parseFloat(str)` | String → Float | `parseFloat("3.14")` → `3.14` |
| `Number(value)` | Any → Number | `Number("42")` → `42` |
| `+value` | Unary plus | `+"42"` → `42` |
...
console.log("  '42px' →", parseInt("42px"));     // Stops at 'p'
console.log("  '3.99' →", parseInt("3.99"));     // Truncates decimal
console.log("  'abc' →", parseInt("abc"));       // NaN
console.log("  '  42  ' →", parseInt("  42  ")); // Trims whitespace

console.log("\\nRadix (base) examples:");
console.log("  '10' base 2 →", parseInt("10", 2));   // Binary: 2
console.log("  '10' base 8 →", parseInt("10", 8));   // Octal: 8
console.log("  '10' base 16 →", parseInt("10", 16)); // Hex: 16
console.log("  'ff' base 16 →", parseInt("ff", 16)); // Hex: 255

console.log("\\nparseFloat examples:");
console.log("  '3.14' →", parseFloat("3.14"));
console.log("  '3.14.15' →", parseFloat("3.14.15")); // Stops at second dot
console.log("  '314e-2' →", parseFloat("314e-2"));   // Scientific notation

console.log("\\nNumber() vs parseInt():");
console.log("  Number('42px') →", Number("42px"));   // NaN (stricter)
...
setTimeout(() => {
  console.log("⏰ 1000ms timeout fired!");
}, 1000);
...
const intervalId = setInterval(() => {
  ...
  if (count >= 3) {
    clearInterval(intervalId);
    console.log(" Interval stopped after 3 ticks");
  }
}, 400);
