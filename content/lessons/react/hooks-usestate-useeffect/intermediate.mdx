...
```jsx
// ❌ BAD - Order changes based on condition
...
// ❌ BAD - One giant effect doing everything
function UserDashboard({ userId }) {
  useEffect(() => {
    fetchUser(userId).then(setUser);
    document.title = \`Dashboard - \${user?.name}\`;
...
**Excellent progress!**  You now understand the nuances of useEffect and dependency management. In the advanced level, we'll explore custom hooks, useCallback, useMemo, useRef, and performance optimization techniques.
