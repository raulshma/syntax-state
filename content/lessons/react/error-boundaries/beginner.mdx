...
Welcome to Error Boundaries! Have you ever seen an app suddenly crash and show a blank white screen? That's what happens when React encounters an error it can't handle. Let's learn how to prevent that! ️
...
function BuggyComponent() {
  ...
  return (
    <ul>
      {users.map(user => (  // 💥 CRASH! Can't call .map on undefined
...
**Great job!**  You now understand the basics of error boundaries. In the intermediate level, we'll dive deeper into `getDerivedStateFromError`, `componentDidCatch`, and how to create better fallback UIs.
