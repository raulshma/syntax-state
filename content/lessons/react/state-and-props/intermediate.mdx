...
```jsx
// Data needs to go from App → Layout → Sidebar → UserInfo
function App() {
...
**Great progress!**  You've learned important patterns for managing state in React applications. In the advanced level, we'll explore state batching, the reconciliation algorithm, and performance optimization techniques.
