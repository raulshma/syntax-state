...
## The `let` and `const` Elevator - Strict Security 
...
```javascript
// console.log(name); // ❌ ERROR! Can't access before declaration
let name = "Alice";
...
## Function Hoisting - The Super VIP 
...
greet(); // ✅ "Hi!" (only works after declaration)
```
...
| :--------------- | :-------- | :--------------- | :----------------- |
| `var`            | Yes     | `undefined`      | Old code only      |
| `let`            | Yes     | No (TDZ)       | When value changes |
| `const`          | Yes     | No (TDZ)       | Default choice     |
| `function`       | Yes     | Yes            | Regular functions  |
| Arrow `=>`       | Depends\* | No             | Modern code        |
