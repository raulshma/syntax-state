...
1. **Tokenization** — The raw HTML string is converted into tokens (start tags, end tags, text, comments)
2. **Tree Construction** — Tokens are assembled into the DOM (Document Object Model)
3. **Error Handling** — Invalid markup is corrected using well-defined rules

```
HTML String → Tokenizer → Tokens → Tree Builder → DOM Tree
```
...
HTML parsers are designed to handle malformed markup gracefully. This is why "broken" HTML still renders — but don't rely on it!
</KeyConcept>
...
- **Elements and attributes** — What they mean and how they behave
- **Parsing algorithm** — Exactly how browsers should parse HTML
- **DOM interfaces** — JavaScript APIs for interacting with elements
- **Rendering hints** — Default styles and behaviors
...
`<script>` and `<style>` contain raw text — HTML entities aren't parsed:
...
While the outline algorithm was part of the HTML5 spec, browsers never implemented it. Best practice: use explicit heading levels (h1 → h2 → h3) rather than relying on sectioning.
</InfoBox>
...
Modern browsers use **speculative parsing** — while waiting for a blocking script, they scan ahead for resources to preload:
...
**Outstanding work!**  You now have a deep understanding of HTML parsing and the specification. This knowledge will help you write cleaner markup and debug complex issues. Next, explore semantic HTML to make your pages more accessible and SEO-friendly!
