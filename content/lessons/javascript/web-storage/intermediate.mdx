...
The storage event only fires in OTHER tabs/windows. The tab that made the change doesn't receive the event. This is by design — you already know what you changed!
</KeyConcept>
...
- **Circular references** — Objects that reference themselves can't be stringified
- **Functions** — JSON.stringify ignores functions
- **undefined** — Becomes null in JSON
- **Dates** — Become strings, need manual conversion back
</InfoBox>
...
- **httponly** — Can't be accessed by JavaScript (server-set only)
</KeyConcept>
...
**Excellent work!**  You now understand storage events, cookies, and advanced patterns. In the advanced level, we'll explore IndexedDB, the Cache API, and security considerations!
