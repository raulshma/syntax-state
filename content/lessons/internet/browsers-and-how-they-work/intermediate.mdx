...
In this lesson, we'll explore the **rendering pipeline** — the step-by-step process browsers use to turn HTML, CSS, and JavaScript into pixels on your screen.
...
The browser parses HTML and constructs the **Document Object Model (DOM)** — a tree structure representing every element on the page.
...
While building the DOM, the browser also parses CSS and builds the **CSS Object Model (CSSOM)** — a tree of styles.
...
The browser combines DOM and CSSOM to create the **Render Tree** — only visible elements with their computed styles.
...
The browser fills in pixels — drawing text, colors, images, borders, and shadows.
...
Elements on separate compositor layers can be animated by the GPU without triggering layout or paint — this is why `transform` animations are so smooth.
</KeyConcept>
...
|-------------|----------|
| Content change | Layout → Paint → Composite |
| Style change (geometry) | Layout → Paint → Composite |
| Style change (visual only) | Paint → Composite |
...
Use `requestAnimationFrame` for animations — it syncs with the browser's refresh rate (usually 60fps) for smooth visuals.
</InfoBox>
...
**Excellent progress!**  You now understand the rendering pipeline in detail. In the advanced level, we'll explore JavaScript engines, GPU acceleration, and browser security models.
