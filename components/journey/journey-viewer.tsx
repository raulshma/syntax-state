...
import { JourneyFlowNodeMemo, type JourneyFlowNodeData, type JourneyFlowNode } from './journey-flow-node';
...
const LAYOUT_STORAGE_KEY = 'Journey-elk-layout-preference';
const getViewportStorageKey = (JourneySlug: string) => `Journey-viewport-${JourneySlug}`;
const getSelectedNodeStorageKey = (JourneySlug: string) => `Journey-selected-node-${JourneySlug}`;
...
// Custom node types
const nodeTypes = {
  JourneyNode: JourneyFlowNodeMemo,
};
...
interface JourneyViewerInnerProps extends JourneyViewerProps {
...
function JourneyViewerInner({
  ...
  const initialNodes = useMemo((): JourneyFlowNode[] => {
    if (!layoutResult) return [];
    
    return journey.nodes.map((node) => {
      const layoutNode = layoutResult.nodes.find(n => n.id === node.id);
      // Get sub-Journey progress if this node has a sub-Journey (Requirements: 4.2, 4.3)
      const subJourneyProgress = node.subJourneySlug 
        ? subJourneyProgressMap[node.subJourneySlug] 
        : undefined;
  ...
  }, [journey.nodes, getNodeStatus, selectedNodeId, onNodeClick, layoutResult, subJourneyProgressMap]);

  // Convert Journey edges to React Flow format
...
export function JourneyViewer({ subJourneyProgressMap = {}, ...props }: JourneyViewerProps) {
  const [layoutType, setLayoutType] = useState<ElkLayoutType>('mrtree');
  ...
  useEffect(() => {
    ...
    // Load saved viewport
    const viewportKey = getViewportStorageKey(props.journey.slug);
    ...
    const selectedNodeKey = getSelectedNodeStorageKey(props.journey.slug);
    const savedSelectedNode = localStorage.getItem(selectedNodeKey);
    if (savedSelectedNode && props.journey.nodes.some(n => n.id === savedSelectedNode)) {
      // Call onNodeClick to restore selection using ref to avoid stale closure
      onNodeClickRef.current(savedSelectedNode);
    }

    setHasLoadedState(true);
  }, [props.journey.slug, props.journey.nodes]);
  ...
  }, [props.selectedNodeId, props.journey.slug, hasLoadedState]);
  ...
  // Compute layout when type or journey changes
  useEffect(() => {
    let cancelled = false;
    
    async function runLayout() {
      // 1. Check cache first (before setting loading state)
      const cacheKey = `Journey-layout-${props.journey.slug}-${layoutType}`;
      const versionHash = `${props.journey.nodes.length}-${props.journey.edges.length}`; // Simple versioning
      
      try {
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
          const parsed = JSON.parse(cached);
          // Check if cache matches current Journey version
  ...
  }, [props.journey.nodes, props.journey.edges, layoutType, props.journey.slug]);
